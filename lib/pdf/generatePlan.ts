...
function blockHeight(block: PlanBlock, f: F, blockIndex = 0): number {
  ...
  // so the renderer never crashes on missing items[].
...
function drawBlock(
  ...
  // Defensive guard against unknown/deprecated block shapes.
  if (!Array.isArray(block.items)) return startY;
...
function drawKeyTakeaways(
  ...
  const actions = plan.blocks.filter((b): b is LegacyPlanBlock => Array.isArray(b.items)).map((b) => b.items[0]).filter(Boolean);
...
function buildPlanContent(doc: PDFDocument, plan: PlanPdfInput, accentColor: Color, f: F, today: string): void {
  ...
  {
    const ktActions = plan.blocks.filter((b): b is LegacyPlanBlock => Array.isArray(b.items)).map((b) => b.items[0]).filter(Boolean);
