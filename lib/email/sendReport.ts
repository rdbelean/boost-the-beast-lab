...
function buildHtml(downloadUrl: string, scores: ScoreSummary): string {
      ...
      <table role="presentation" cellspacing="0" cellpadding="0" width="600" style="max-width:600px;background:#0D0D0F;">
        ...
        <tr><td style="padding:0 0 30px 0;">
          <h1 style="font-family:Arial Black,sans-serif;font-size:42px;font-weight:900;letter-spacing:-0.02em;color:#fff;line-height:1.05;margin:0;">
            DEIN REPORT<br/>IST BEREIT.
          </h1>
        </td></tr>

        <tr><td style="padding:24px 0;background:#16161A;border-left:3px solid #E63222;text-align:center;">
          <div style="font-size:10px;color:#8a8a92;letter-spacing:0.25em;text-transform:uppercase">OVERALL PERFORMANCE INDEX</div>
        ...
        <tr><td style="padding:30px 0 10px 0;">
          <table role="presentation" cellspacing="0" cellpadding="0" width="100%">
            ${row("Activity", scores.activity)}
            ${row("Sleep", scores.sleep)}
            ${row("VO2max", scores.vo2max)}
            ${row("Metabolic", scores.metabolic)}
            ${row("Stress", scores.stress)}
          </table>
        </td></tr>
...
export async function sendReportEmail(
  ...
  const { error } = await resend.emails.send({
    ...
    html: buildHtml(downloadUrl, scores),
  });
  if (error) {
