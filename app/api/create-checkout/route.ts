...
const PRODUCTS: Record<Locale, Record<string, ProductCopy & { price: number }>> = {
  ...
  tr: {
    metabolic: { name: "Metabolic Performance Score", price: 2900, description: "BMI, beslenme & hidrasyon skoru, AI raporu, premium PDF" },
    recovery: { name: "Recovery & Regeneration Score", price: 2900, description: "Uyku analizi, toparlanma skoru, AI raporu, premium PDF" },
    "complete-analysis": { name: "Complete Performance Analysis", price: 3990, description: "4 skorun tamamı + genel endeks, AI raporu, 30 günlük tahmin, premium PDF" },
    "plan-metabolic": { name: "Metabolic Boost Plan", price: 2499, description: "30 günlük kişisel beslenme & metabolik optimizasyon planı" },
    "plan-recovery": { name: "Recovery Protocol", price: 2499, description: "Maksimum toparlanma için kişisel uyku & recovery protokolü" },
    "plan-activity": { name: "Performance Training Plan", price: 2499, description: "12 haftalık kişisel kuvvet & kondisyon antrenman planı" },
    "plan-stress": { name: "Stress Reset Program", price: 2499, description: "30 günlük anti-stres & yaşam tarzı optimizasyon programı" },
    "bundle-all": { name: "4 Optimizasyon Planı — Paket", price: 4999, description: "Metabolic + Recovery + Activity + Stress — eksiksiz performans optimizasyonu" },
  },
};
...
const TOS_MESSAGE: Record<Locale, string> = {
  ...
  tr: "Onayla, Boost the Beast Lab'in ödeme bilgilerini saklayabileceğini ve sonuç sayfasında izin vereceğin opsiyonel upsell alımları (antrenman ve beslenme planları) için kullanabileceğini kabul etmiş olursun.",
};
...
const ERROR_MSG: Record<Locale, { not_found: string; checkout_failed: string }> = {
  ...
  tr: { not_found: "Ürün bulunamadı", checkout_failed: "Ödeme başarısız" },
};
